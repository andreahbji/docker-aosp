...
if [ "$1" = "docker" ]; then
    ...
    TEST_URL=${TEST_URL:-git://shanghai.source.codeaurora.org/platform/manifest.git}
    TEST_MANIFEST=${TEST_MANIFEST:-LNX.LA.3.7.2.c6-10200-8x16.0.xml}
    TEST_MODEL=${TEST_MODEL:-msm8916_32-userdebug}

    cpus=$(grep ^processor /proc/cpuinfo | wc -l)

    repo init --depth 1 -u "$TEST_URL" -b "$TEST_BRANCH" -m "$TEST_MANIFEST" --repo-url=git://shanghai.source.codeaurora.org/tools/repo.git --repo-branch=caf-stable
    ...
    export AOSP_IMAGE="andreaji/aosp:4.4-kitkat"
